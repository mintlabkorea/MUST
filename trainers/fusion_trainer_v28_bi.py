...
class FusionTrainer(nn.Module, dataProcessor):
    def __init__(self, cfg: Config, train_keys, val_keys, test_keys):
        ...
        # [수정됨] 모델 빌드와 가중치 로드만 수행. 옵티마이저 생성은 fusion_train에서 단계별로 진행.
        self._build_model()
    ...
    def _load_pretrained_weights(self):
        ...
        if getattr(emo_cfg, 'modalities_to_use', []):
            emo_ckpt_path = getattr(emo_cfg, 'ckpt_path', 'weights/best_emotion_ppg_sc_survey_bi.pt')
    ...
    def evaluate(self, loader, return_preds=False):
        ...
        for batch in loader:
            ...
            p_v_ternary[(p_v_multiclass >= 0) & (p_v_multiclass <= 3)] = 0  # Low: 1,2,3점
            p_v_ternary[(p_v_multiclass >= 4) & (p_v_multiclass <= 8)] = 1  # Medium: 4,5,6점
            
            t_v_ternary = torch.full_like(t_v_multiclass, -1)
            t_v_ternary[(t_v_multiclass >= 0) & (t_v_multiclass <= 3)] = 0
            t_v_ternary[(t_v_multiclass >= 4) & (t_v_multiclass <= 8)] = 1
            ...
            p_a_ternary[(p_a_multiclass >= 0) & (p_a_multiclass <= 3)] = 0
            p_a_ternary[(p_a_multiclass >= 4) & (p_a_multiclass <= 8)] = 1
            
            t_a_ternary = torch.full_like(t_a_multiclass, -1)
            t_a_ternary[(t_a_multiclass >= 0) & (t_a_multiclass <= 3)] = 0
            t_a_ternary[(t_a_multiclass >= 4) & (t_a_multiclass <= 8)] = 1
    ...
    # [V24에서 가져옴] 2단계 학습을 총괄하는 메인 훈련 함수
    def fusion_train(self, save_path="weights/best_fusion_v28.pt"): 
        ...
        encoder_params = itertools.chain(self.nets.parameters(), self.projs.parameters())
        # [핵심] 기존 옵티마이저에 인코더 파라미터 그룹을 추가합니다.
