"""
v30: Group Alternating Training
- 시퀀스 그룹 (Motion+ACT)과 단일 값 그룹 (Emotion+TOT)으로 분리
- 두 그룹을 에폭마다 번갈아 학습하여 안정성과 성능을 모두 추구하는 최종 버전
"""
import os
import torch
...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _build_model(self):
        ...
        # TOT/ACT 헤드 정의
        final_fusion_dim = hidden_dim # final_motion_tokens의 H 차원
        
        # TOT Head (분류)
        num_tot_classes = 3  # 예: 0: 안전, 1: 주의, 2: 긴급
        ...
        # log_vars 크기를 6으로 변경 (Motion, Valence, Arousal, Align, TOT, ACT)
    ...
    def _set_task_grad(self, task_group: str):
        """ 'sequence' 또는 'scalar' 그룹에 따라 헤드의 그래디언트를 설정 """
        
        # 시퀀스 예측 그룹 (행동, ACT)
        seq_params = itertools.chain(self.motion_head.parameters(), self.act_head.parameters())
        for param in seq_params:
            param.requires_grad = (task_group == 'sequence')

        # 단일 값 예측 그룹 (감정, TOT, 정렬)
        scalar_params = itertools.chain(
            self.emotion_valence_predictor.parameters(),
            self.emotion_arousal_predictor.parameters(),
            self.alignment_head.parameters(),
            self.tot_head.parameters()
        )
        for param in scalar_params:
            param.requires_grad = (task_group == 'scalar')
    ...
    def forward(self, batch):
        ...
        T_mot = batch['label_motion'].shape[1] if 'label_motion' in batch \
                and batch['label_motion'].dim() == 2 else 1000
        ...
        # ACT는 전체 시퀀스에 대해 예측
        act_pred = self.act_head(final_motion_tokens).squeeze(-1)

        # TOT는 마지막 시점의 특징으로 예측
        last_step_features = final_motion_tokens[:, -1, :]
    ...
    def run_epoch(self, loader, train: bool, current_task_group: str):
        self.train(train)
        total_loss, num_batches = 0.0, 0
        device = self.cfg.Project.device

        if train:
            self._set_task_grad(current_task_group)

        for batch in loader:
            # (batch to device 로직 생략)

            with torch.set_grad_enabled(train):
                out = self.forward(batch)
                loss = torch.tensor(0., device=device)

                if current_task_group == 'sequence':
                    # Motion 손실
                    mot_labels = batch['label_motion'].to(device)
                    valid_mask = (mot_labels.reshape(-1) > 0) & (mot_labels.reshape(-1) != 4)
                    loss_mot = F.cross_entropy(out['motion_logits'].reshape(-1, out['motion_logits'].shape[-1])[valid_mask], (mot_labels.reshape(-1)[valid_mask] - 1).long()) if valid_mask.any() else torch.tensor(0., device=device)
                    
                    # ACT 손실
                    act_true_labels = batch['label_act'].to(device).squeeze(-1)
                    loss_act = self._calculate_act_loss(out['act_pred'], act_true_labels)
                    
                    loss = loss_mot + self.cfg.MainTask.get('lambda_act', 1.0) * loss_act

                elif current_task_group == 'scalar':
                    # Emotion 손실
                    ...
                    # TOT 손실
                    loss_tot = self.tot_loss_fn(out['tot_logits'], batch['label_tot'].to(device))

                    loss = (self.cfg.MainTask.lambda_emotion * (loss_v + loss_a) +
                            self.cfg.MainTask.cross_modal_lambda * loss_align +
                            self.cfg.MainTask.get('lambda_tot', 1.0) * loss_tot)
    ...
    def evaluate(self, loader, return_preds=False):
        self.eval()
        all_preds = {
            'motion_preds': [], 'motion_trues': [], 
            'valence_preds': [], 'valence_trues': [], 
            'arousal_preds': [], 'arousal_trues': [],
            'tot_preds': [], 'tot_trues': [],
            'act_preds': [], 'act_trues': []
        }
        for batch in loader:
           ...
           # TOT 평가
            p_tot = out['tot_logits'].argmax(-1).cpu()
            t_tot = batch['label_tot'].cpu()
            valid_mask_tot = t_tot != -100
            if valid_mask_tot.any():
                all_preds['tot_preds'].append(p_tot[valid_mask_tot])
                all_preds['tot_trues'].append(t_tot[valid_mask_tot])

            # ACT 평가
            p_act = out['act_pred'].cpu()
            t_act = batch['label_act'].squeeze(-1).cpu()
            valid_mask_act = t_act != -100.0
            if valid_mask_act.any():
                all_preds['act_preds'].append(p_act[valid_mask_act])
                all_preds['act_trues'].append(t_act[valid_mask_act])

        # 모든 텐서 합치기 전 비어있는 리스트 확인
        for key in all_preds:
            if not all_preds[key]: # 리스트가 비어있으면
                print(f"Warning: No valid labels found for '{key}' during evaluation.")
                return 0.0, 0.0, 0.0, 0.0, 999.9 # 기본값 반환
        ...
        acc_tot = accuracy_score(all_preds['tot_trues'], all_preds['tot_preds'])
        mse_act = mean_squared_error(all_preds['act_trues'], all_preds['act_preds'])
        
        return acc_mot, acc_v, acc_a, acc_tot, mse_act
        
    # 2단계 학습을 총괄하는 메인 훈련 함수
    def fusion_train(self, save_path="weights/best_fusion_v29_grouped_alt.pt"):
        ...
        warmup_epochs = getattr(self.cfg.MainTask, 'warmup_epochs', 15)
        ...
        # --- 1단계: 워밍업 (Warm-up) ---
        print("\n" + "="*50)
        print(f"🚀 STARTING STAGE 1: WARM-UP FOR {warmup_epochs} EPOCHS (GROUP ALTERNATING)")
        print("="*50)
        self._set_encoder_grad(requires_grad=False)
        self._create_optimizer_stage1()

        for epoch in range(1, warmup_epochs + 1):
            current_task_group = 'sequence' if epoch % 2 != 0 else 'scalar'
            print(f"\n[WARM-UP] Epoch {epoch:02d}/{warmup_epochs} | Current Task Group: {current_task_group.upper()}")
            
            tr_loss = self.run_epoch(tr_loader, train=True, current_task_group=current_task_group)
            va_loss = self.run_epoch(va_loader, train=False, current_task_group=current_task_group)
            
            va_mot, va_v, va_a, va_tot, va_act_mse = self.evaluate(va_loader)
            print(f"  -> Tr Loss({current_task_group}): {tr_loss:.4f} | Val Loss({current_task_group}): {va_loss:.4f}")
            print(f"  -> Val Acc(M/V/A/T): {va_mot:.3f}/{va_v:.3f}/{va_a:.3f}/{va_tot:.3f} | ACT_MSE: {va_act_mse:.4f}")
            
            current_score = (va_mot + va_v + va_a + va_tot) / 4

            if current_score > best_score:
                best_score = current_score
                patience_counter = 0
                best_performance = {'mot_acc': va_mot, 'val_acc': va_v, 'aro_acc': va_a, 'tot_acc': va_tot, 'act_mse': va_act_mse}
                torch.save({'model_state_dict': self.state_dict()}, save_path)
                print(f"  -> Best warm-up model saved! Avg Acc: {best_score:.4f}")
            else:
                patience_counter += 1

        # --- 2단계: 미세 조정 (Fine-tuning) ---
        print("\n" + "="*50)
        print(f"🚀 STARTING STAGE 2: FINE-TUNING FOR {total_epochs - warmup_epochs} EPOCHS (GROUP ALTERNATING)")
        print("="*50)
        ...
        encoder_lr = self.cfg.MainTask.lr / 20
        encoder_params = itertools.chain(self.nets.parameters(), self.projs.parameters())
        self.optim.add_param_group({
            'params': encoder_params, 'lr': encoder_lr, 
            'weight_decay': self.cfg.MainTask.weight_decay, 'name': 'Encoders (Low LR)'
        })
        
        patience_counter = 0
        for epoch in range(warmup_epochs + 1, total_epochs + 1):
            current_task_group = 'sequence' if epoch % 2 != 0 else 'scalar'
            print(f"\n[FINE-TUNE] Epoch {epoch:02d}/{total_epochs} | Current Task Group: {current_task_group.upper()}")
            ...
            if current_score > best_score:
                ...
                print(f"  -> Best fine-tuned model saved! Avg Acc: {best_score:.4f}")
            else:
                patience_counter += 1
        ...
        print(f"\n--- Best Validation Performance (Staged & Group Alternating Training) ---")
        print(f"Best Avg Accuracy: {best_score:.4f}")
        print(f"Acc(M/V/A/T): {best_performance.get('mot_acc', 0):.3f}/{best_performance.get('val_acc', 0):.3f}/{best_performance.get('aro_acc', 0):.3f}/{best_performance.get('tot_acc', 0):.3f} | ACT_MSE: {best_performance.get('act_mse', 0):.4f}")
