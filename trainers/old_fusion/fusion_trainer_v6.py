# v5에서 모션 퓨전레이어 얇게. 헤드별 최적 가중치 로드.
...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _build_model(self):
        ...
        veh_encoder_emotion = VehicleTCNEncoder(self.cfg) # 감정용 (12채널)
        original_veh_dim = self.cfg.veh_params['input_dim']
        self.cfg.veh_params['input_dim'] = 1 # 모션용 (1채널)
    ...
    def _create_optimizer(self):
        ...
        # config의 fusion_params 설정을 읽어 학습할 모듈을 동적으로 추가
        if self.cfg.fusion_params["train_sm_fuse"]:
            trainable_params_list.append(self.sm_fuse.parameters())
            trainable_params_list.append(self.bg_head.parameters())
            trainable_params_list.append(self.eg_val_head.parameters())
            trainable_params_list.append(self.eg_aro_head.parameters())
            print("Optimizer will train: SMFusion path")

        if self.cfg.fusion_params["train_gm_fuse"]:
            trainable_params_list.append(self.gm_fuse.parameters())
            print("Optimizer will train: GMFusion block")

        if self.cfg.fusion_params["train_motion_predictor"]:
            trainable_params_list.append(self.motion_predictor.parameters())
            print("Optimizer will train: MotionPredictor")

        if self.cfg.fusion_params["train_emotion_predictor"]:
            trainable_params_list.append(self.emotion_predictor.parameters())
            print("Optimizer will train: EmotionPredictor")

        # 학습 가능한 손실 가중치는 항상 학습 대상에 포함
        # non-leaf Tensor 에러를 방지하기 위해 리스트로 감싸서 추가
        trainable_params_list.append([self.log_vars])
        print("Optimizer will train: Learnable loss weights (log_vars)")

        # 최종적으로 학습 대상이 된 모든 파라미터를 하나로 묶음
        self.optim = torch.optim.Adam(itertools.chain(*trainable_params_list), lr=self.cfg.lr)
    ...
    def forward(self, batch):
        ...
        # 2-a: 모션 경로
        # 모션 관련 특징만 선별하여 Predictor에 전달
        motion_input_dict = {
            'imu': imu_emb.unsqueeze(1),
            'veh': veh_emb_m.unsqueeze(1), # 모션 전문가 veh_emb 사용
            'sc': sc_emb.unsqueeze(1)
        }
        mot_repr, mot_logits = self.motion_predictor(motion_input_dict, return_feature=True)
        
        # 2-b: 감정 경로
        # 감정 경로는 감정 전문가 veh_emb를 사용
        features_dict = {'imu': imu_emb, 'ppg': ppg_emb, 'veh': veh_emb_e, 'sc': sc_emb, 'survey': survey_emb}
        ...
        emotion_input_dict = {k: v.unsqueeze(1) for k, v in features_dict.items()}
        emotion_input_dict['fused'] = fused_global.unsqueeze(1)
        
        # 감정 예측기는 모션 경로에서 온 깨끗한 mot_repr를 context로 사용
        emo_dict = self.emotion_predictor(emotion_input_dict, context=mot_repr.detach())
        
        return {
            'motion_logits': mot_logits.squeeze(1),
            'valence_logits': emo_dict['valence_logits'],
            'arousal_logits': emo_dict['arousal_logits'],
            'bg_logits': bg_logits,
