"""
v19 기반, Asymmetric Alignment Loss 적용
- 기존 loss_cross를 'motion 특징으로 emotion 특징을 예측'하는 단방향 loss_align으로 대체합니다.
- .detach()를 사용해 그래디언트가 motion으로 역전파되지 않도록 막는 것이 핵심입니다.
"""
import torch
...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _build_model(self):
        print("\n--- Building V21 model architecture: Bi-directional FiLM + Causal Context ---")
    ...
    def _create_optimizer(self):
        params_to_train = []
        # 기존 학습 파라미터들
        if hasattr(self, 'motion_feature_fusion'): params_to_train.append(self.motion_feature_fusion.parameters())
        if hasattr(self, 'emotion_feature_fusion'): params_to_train.append(self.emotion_feature_fusion.parameters())
        if hasattr(self, 'motion_head'): params_to_train.append(self.motion_head.parameters())
        if hasattr(self, 'emotion_valence_predictor'): params_to_train.append(self.emotion_valence_predictor.parameters())
        if hasattr(self, 'emotion_arousal_predictor'): params_to_train.append(self.emotion_arousal_predictor.parameters())
        
        # Cross-attention, FiLM 파라미터들
        params_to_train += [
            ...
            self.mot_film.parameters(), 
        ]
        params_to_train.append(self.alignment_head.parameters())
    ...
    def run_epoch(self, loader, train: bool):
        ...
        for batch in loader:
            with torch.set_grad_enabled(train):
                ...
                # fused_motion에 .detach()를 적용하여 그래디언트 흐름 차단
                fused_motion_detached = out['fused_motion'].detach()
                # alignment_head를 통해 emotion 특징을 예측
                predicted_emotion = self.alignment_head(fused_motion_detached)
                # 예측된 값과 실제 fused_emotion 사이의 MSE 손실 계산
    ...
    def fusion_train(self):
        ...
        for epoch in range(1, self.cfg.MainTask.epochs + 1):
            ...
            if va_loss < best_loss:
                ...
                }, "weights/best_fusion_v21.pt") # Updated save path
            else:
                patience_counter += 1
        ...
        print(f"\n--- Best Validation Performance (v21) ---")
