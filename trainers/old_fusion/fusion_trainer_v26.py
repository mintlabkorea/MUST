...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _build_model(self):
        print("\n--- Building V26 model architecture: Bi-directional FiLM + Causal Context ---")
        ...
        self.mot_prior_temperature = getattr(self.cfg.FusionModel, "mot_prior_temperature", 2.0)  # ⬅️ 추가
    ...
    def forward(self, batch):
        ...
        if self.emo_use_mot_logits:
            T = float(self.mot_prior_temperature)                     # ⬅️ 추가
            p_mot = F.softmax(mot_logits / T, dim=-1).mean(dim=1).detach()  # ⬅️ 변경
    ...
    def _compute_motion_weights(self, loader):
        device = self.cfg.Project.device
        all_m = []
        for batch in loader:
            m = batch['label_motion'].view(-1).cpu().numpy()  # 0=pad, 1..4=유효
            all_m.append(m)
        m_all = np.concatenate(all_m)
        mask = (m_all > 0)                # 1..4만 카운트
        tgt  = m_all[mask] - 1            # 0..3
        C = int(self.cfg.PretrainMotion.num_motion)  # 3 또는 4
        cnt = np.bincount(tgt, minlength=C).astype(np.float64)
        cnt[cnt == 0] = 1.0
        w = (1.0 / cnt)
        w = (w / w.mean()).astype(np.float32)
        self.motion_weights = torch.tensor(w, device=device)
        print("[Motion weights]", w)

    def run_epoch(self, loader, train: bool):
        self.train(train)
        total_loss, num_batches = 0.0, 0
        device = self.cfg.Project.device
        drive_label = getattr(self.cfg.PretrainMotion, 'drive_label', 4)  # ⬅️ Drive 라벨

        for batch in loader:
            with torch.set_grad_enabled(train):
                out = self.forward(batch)

                # -------- Motion (1~3만 학습, 0은 패딩, 4는 제외) --------
                mot_logits = out['motion_logits']                  # (B,T,C=3)
                mot_labels = batch['label_motion'].to(device)      # (B,T)
                flat = mot_labels.reshape(-1)
                valid_mask = (flat > 0) & (flat != drive_label)    # ⬅️ Drive 제외
                target_mot = (flat[valid_mask] - 1).long()         # 1..3 → 0..2

                weight = getattr(self, "motion_weights", None)
                # 안전장치: weight 길이가 로짓 클래스 수와 다르면 가중치 미적용
                if weight is not None and weight.numel() != mot_logits.shape[-1]:
                    weight = None
                ...
                # -------- Valence (4,7 고정 + NaN/Inf 방어, 가중치 없음) --------
                val_logits = out['valence_logits']                 # (N,3) 또는 (B,3)
    ...
    def evaluate(self, loader, return_preds=False):
        self.eval()
        drive_label = getattr(self.cfg.PretrainMotion, 'drive_label', 4)
        ...
        for batch in loader:
            ...
            # ---- Motion (Drive 제외) ----
            p_m = out['motion_logits'].argmax(-1).cpu()   # (B,T)
            t_m_raw = batch['label_motion'].cpu()         # (B,T)
            msk = (t_m_raw > 0) & (t_m_raw != drive_label)
            all_preds['motion_preds'].append(p_m[msk])
            all_preds['motion_trues'].append((t_m_raw[msk] - 1))   # 0..2

            # ---- Valence ----
            p_v = out['valence_logits'].argmax(-1).cpu()
            raw_v = torch.nan_to_num(batch['valence_reg_emotion'].view(-1).cpu(),
                                    nan=0.0, posinf=0.0, neginf=0.0)
            ...
            t_v[raw_v >= 7] = 2
            keep_v = (t_v != -1)
            all_preds['valence_preds'].append(p_v[keep_v])
            all_preds['valence_trues'].append(t_v[keep_v])

            # ---- Arousal ----
            p_a = out['arousal_logits'].argmax(-1).cpu()
            raw_a = torch.nan_to_num(batch['arousal_reg_emotion'].view(-1).cpu(),
                                    nan=0.0, posinf=0.0, neginf=0.0)
            ...
            t_a[raw_a >= 7] = 2
            keep_a = (t_a != -1)
            all_preds['arousal_preds'].append(p_a[keep_a])
            all_preds['arousal_trues'].append(t_a[keep_a])
        ...
        acc_m = accuracy_score(all_preds['motion_trues'], all_preds['motion_preds'])
        f1_m  = f1_score(all_preds['motion_trues'], all_preds['motion_preds'], average='macro')
        b_m   = balanced_accuracy_score(all_preds['motion_trues'], all_preds['motion_preds'])
        ...
        b_v   = balanced_accuracy_score(all_preds['valence_trues'], all_preds['valence_preds'])
        ...
        b_a   = balanced_accuracy_score(all_preds['arousal_trues'], all_preds['arousal_preds'])

        return {
            'mot': {'acc': acc_m, 'f1': f1_m, 'bacc': b_m},
            'val': {'acc': acc_v, 'f1': f1_v, 'bacc': b_v},
            'aro': {'acc': acc_a, 'f1': f1_a, 'bacc': b_a},
        }

            
    def fusion_train(self, save_path="weights/best_fusion_v26.pt"):
        ...
        # (참고) motion_weights를 쓰는 경우, 3클래스 기준으로 계산되었는지 확인하세요.
        # self._compute_motion_weights(tr_loader)  # 사용중이라면, 함수 내부가 drive 제외로 집계하도록 수정 권장.

        best_loss, patience_counter, best_perf = float('inf'), 0, {}
        ...
        for epoch in range(1, self.cfg.MainTask.epochs + 1):
            ...
            va = self.evaluate(va_loader)
            print(f"Epoch {epoch:02d} | Tr Loss {tr_loss:.4f} | Val Loss {va_loss:.4f} | "
                f"Acc M/V/A: {va['mot']['acc']:.3f}/{va['val']['acc']:.3f}/{va['aro']['acc']:.3f} | "
                f"F1  M/V/A: {va['mot']['f1']:.3f}/{va['val']['f1']:.3f}/{va['aro']['f1']:.3f} | "
                f"BAcc M/V/A: {va['mot']['bacc']:.3f}/{va['val']['bacc']:.3f}/{va['aro']['bacc']:.3f}")

            if va_loss < best_loss:
                best_loss, patience_counter = va_loss, 0
                best_perf = {
                    'best_loss': best_loss,
                    'mot_acc': va['mot']['acc'], 'val_acc': va['val']['acc'], 'aro_acc': va['aro']['acc'],
                    'mot_f1':  va['mot']['f1'],  'val_f1':  va['val']['f1'],  'aro_f1':  va['aro']['f1'],
                    'mot_bacc':va['mot']['bacc'],'val_bacc':va['val']['bacc'],'aro_bacc':va['aro']['bacc'],
                }
                torch.save({
        ...
        print(f"\n--- Best Validation Performance (v26, drive excluded) ---")
        if best_perf:
            print(f"Loss: {best_perf['best_loss']:.4f} | "
                f"Acc M/V/A: {best_perf['mot_acc']:.3f}/{best_perf['val_acc']:.3f}/{best_perf['aro_acc']:.3f} | "
                f"F1  M/V/A: {best_perf['mot_f1']:.3f}/{best_perf['val_f1']:.3f}/{best_perf['aro_f1']:.3f} | "
                f"BAcc M/V/A: {best_perf['mot_bacc']:.3f}/{best_perf['val_bacc']:.3f}/{best_perf['aro_bacc']:.3f}")
        return best_perf
