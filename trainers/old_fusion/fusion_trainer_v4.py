...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _build_model(self):
        ...
        self.projs = nn.ModuleDict({
            ...
            'ppg': nn.Linear(self.cfg.ppg_params['embed_dim'] + 6, self.cfg.hidden), 
        ...
        # 손실 함수 및 스케일러 정의
        initial_biases = torch.tensor([0.0, 0.0, 0.0, 1.6, 1.6, 1.6]) # main_loss + aux_loss (mot,v,a)
        ...
        self.ce_aux = nn.CrossEntropyLoss() # Auxiliary loss용
    ...
    def _load_pretrained_weights(self):
        ...
        # EmotionEncoder의 전체 state_dict을 불러옵니다.
        emotion_encoder_state_dict = ckpt['encoder']
        
        # 1. 'nets.' 로 시작하는 키들만 필터링하여 self.nets를 위한 state_dict을 만듭니다.
        #    'nets.' 라는 접두사를 제거해야 키가 정확히 일치합니다.
        nets_state_dict = {
            k.replace('nets.', '', 1): v
            for k, v in emotion_encoder_state_dict.items()
            if k.startswith('nets.')
        }
        
        # 2. 'projs.' 로 시작하는 키들만 필터링합니다.
        projs_state_dict = {
            k.replace('projs.', '', 1): v
            for k, v in emotion_encoder_state_dict.items()
            if k.startswith('projs.')
        }
        
        # 3. 재구성된 state_dict을 각 모듈에 로드합니다.
        if nets_state_dict:
            self.nets.load_state_dict(nets_state_dict, strict=False)
        if projs_state_dict:
            self.projs.load_state_dict(projs_state_dict, strict=False)
    ...
    def _create_optimizer(self):
        trainable_params = itertools.chain(
            ...
            self.gm_fuse.parameters(),
            self.motion_predictor.parameters(), self.emotion_predictor.parameters(),
            [self.log_vars] # log_vars 추가
        )
    ...
    def forward(self, batch):
        ...
        # 3. TCN 출력과 HRV 특징을 concat
        combined_ppg = torch.cat([ppg_tcn_out, hrv_features], dim=1)

        # 4. Projection Layer 통과
        ppg_emb = self.projs['ppg'](combined_ppg)
        ...
        # 3-B단계: SMFusion을 통한 보조 손실 계산 경로
        ...
        # GMFusion: X_aux를 Query로 사용하여 중간 융합 특징 생성
        fused_global = self.gm_fuse(X_aux, features_dict) # (B, D)

        # 3-C단계: Predictor를 통한 최종 예측 경로
        # Predictor 입력은 (B,1,D) 형태이므로 unsqueeze(1) 필요
        features_seq_dict = {k: v.unsqueeze(1) for k, v in features_dict.items()}
        features_seq_dict['fused'] = fused_global.unsqueeze(1)

        mot_repr, mot_logits = self.motion_predictor(features_seq_dict, return_feature=True)
        # 인과관계: mot_repr를 detach하여 context로 전달
        emo_dict = self.emotion_predictor(features_seq_dict, context=mot_repr.detach(), guided_feature=fused_global.unsqueeze(1))
