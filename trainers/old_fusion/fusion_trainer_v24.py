...
class FusionTrainer(nn.Module, dataProcessor):
    def __init__(self, cfg: Config, train_keys, val_keys, test_keys):
        ...
        # [수정] 모델 빌드와 가중치 로드만 수행. 옵티마이저 생성 등은 fusion_train에서 단계별로 수행.
        self._build_model()
        self._load_pretrained_weights()

    def _build_model(self):
        print("\n--- Building V24 model architecture: 2-Stage Training ---")
    ...
    def fusion_train(self, save_path="weights/best_fusion_v24.pt"): 
        ...
        warmup_epochs = getattr(self.cfg.MainTask, 'warmup_epochs', 0)
