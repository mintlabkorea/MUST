"""
V10 기반, Hard freeze + cross-task(concatenation)만 구현
"""
import torch
...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _create_optimizer(self):
        ...
        if hasattr(self, 'motion_joint_projection'):
            params_to_train.append(self.motion_joint_projection.parameters())
    ...
    def forward(self, batch):
        ...
        # --- 1. Feature Extraction (양쪽 경로 모두 5개 모달리티 처리) ---
        # Motion Path
        if hasattr(self, 'motion_modalities'):
            for mod in self.motion_modalities:
                if mod == 'imu' and 'imu_motion' in batch:
                    ...
                    sc_pooled = self.nets['sc'](batch['sc_motion_evt'].to(device), batch['sc_motion_type'].to(device), batch['sc_motion_phase'].to(device), batch['sc_motion_time'].to(device))
        ...
        if hasattr(self, 'emotion_modalities'):
            for mod in self.emotion_modalities:
                if mod == 'ppg' and 'ppg_emotion' in batch:
                    ...
                    features['emotion']['veh'] = self.projs['veh'](self.nets['veh'](batch['veh_emotion'].to(device).permute(0, 2, 1), return_pooled=True))
                elif mod == 'sc':
                    features['emotion']['sc'] = self.projs['sc'](self.nets['sc'](batch['scenario_evt_e'].to(device), batch['scenario_type_e'].to(device), batch['phase_evt_e'].to(device), batch['scenario_time_e'].to(device)))
                elif mod == 'imu' and 'imu_motion' in batch:
                    imu_len = (batch['imu_motion'].abs().sum(-1) > 0).sum(1)
                    imu_seq_out = self.nets['imu'](batch['imu_motion'].to(device), imu_len)
                    features['emotion']['imu'] = self.projs['imu'](imu_seq_out.mean(dim=1))
                elif mod == 'survey' and 'survey_e' in batch:
        ...
        # --- 3. Cross-Task Prediction (Concat-based) ---
    ...
    def fusion_train(self):
        ...
        for epoch in range(1, self.cfg.MainTask.epochs + 1):
            ...
            # [수정] 개별 Loss를 포함하여 더 상세하게 출력
            tr_loss_str = (f"Tr Loss: {tr_losses['total']:.3f} ("
                           f"M: {tr_losses['motion']:.2f}, "
                           f"V: {tr_losses['valence']:.2f}, "
                           f"A: {tr_losses['arousal']:.2f}, "
                           f"C: {tr_losses['cross']:.2f})")
