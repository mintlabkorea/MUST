...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _build_model(self):
        print("\n--- Building V23 model architecture: Bi-directional FiLM + Causal Context ---")
    ...
    def _freeze_encoders(self):
        # Gradual Unfreezing을 위해 모든 레이어의 동결을 해제합니다.
        print("\n--- Unfreezing all layers for Gradual Fine-tuning ---")
        for param in self.parameters(): # self.parameters()로 한 번에 모든 파라미터 접근
            param.requires_grad = True
            
    def _create_optimizer(self):
        # [최종 수정] Gradual Unfreezing을 위한 차등 학습률 및 가중치 감쇠 설정
        
        base_lr = self.cfg.MainTask.lr                # e.g., 1e-5
        base_wd = self.cfg.MainTask.weight_decay      # e.g., 5e-4
        encoder_lr = base_lr / 10                     # e.g., 1e-6

        # 1. 사전학습된 인코더 및 프로젝션 파라미터 그룹 (낮은 LR)
        encoder_params = itertools.chain(self.nets.parameters(), self.projs.parameters())
        
        # 2. 새로 추가된 Fusion/Head 파라미터 그룹 (기본 LR)
        # [수정] 인코더/프로젝션을 제외한 나머지 파라미터를 명시적으로 그룹화
        fusion_head_params = itertools.chain(
            self.motion_feature_fusion.parameters(),
            self.emotion_feature_fusion.parameters(),
            self.motion_head.parameters(),
            self.emotion_valence_predictor.parameters(),
            self.emotion_arousal_predictor.parameters(),
            self.cross_attn_m2e.parameters(),
            self.cross_ln_m.parameters(),
            self.cross_attn_e2m.parameters(),
            self.cross_ln_e.parameters(),
        ...
        print("\n--- Optimizer created with Differential LR for Gradual Unfreezing ---")
    ...
    def fusion_train(self, save_path="weights/best_fusion_v23.pt"): 
        ...
        print(f"\n--- Best Validation Performance (v23) ---")
