"""
v19 기반, Asymmetric Learning Rate/Regularization 적용
- Emotion -> Motion 경로에 해당하는 파라미터 그룹에 더 낮은 학습률과 강한 정규화를 적용하여 학습 안정성 향상을 시도합니다.
"""
import torch
...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _create_optimizer(self):
        # 파라미터 그룹을 분리하여 다른 학습률과 가중치 감쇠를 적용
        
        base_lr = self.cfg.MainTask.lr
        base_wd = self.cfg.MainTask.weight_decay
        
        # 비대칭적 하이퍼파라미터 설정 (config 파일에서 조정 가능)
        lr_ratio = getattr(self.cfg.MainTask, 'lr_asym_ratio', 0.1) # 기본값: 1/10
        wd_ratio = getattr(self.cfg.MainTask, 'wd_asym_ratio', 2.0) # 기본값: 2배
        
        cautious_lr = base_lr * lr_ratio
        cautious_wd = base_wd * wd_ratio

        # 그룹 1: Standard Parameters (안정적인 경로 및 예측 헤드)
        params_standard = []
        ...
        # 그룹 2: Cautious Parameters (복잡성을 더하는 Emotion -> Motion 경로)
        params_cautious = []
        ...
        optim_groups = [
            {
                'params': itertools.chain(*params_standard), 
                'lr': base_lr, 
                'weight_decay': base_wd,
                'name': 'Standard Group'
            },
            {
                'params': itertools.chain(*params_cautious), 
                'lr': cautious_lr, 
                'weight_decay': cautious_wd,
                'name': 'Cautious Group'
            },
            {
    ...
    def fusion_train(self):
        ...
        for epoch in range(1, self.cfg.MainTask.epochs + 1):
            ...
            if va_loss < best_loss:
                ...
                }, "weights/best_fusion_v20.pt") # Updated save path
            else:
                patience_counter += 1
        ...
        print(f"\n--- Best Validation Performance (v20) ---")
