...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _create_optimizer(self):
        # v20의 비대칭적 학습률/정규화 전략을 적용
        base_lr = self.cfg.MainTask.lr
        base_wd = self.cfg.MainTask.weight_decay
        
        lr_ratio = getattr(self.cfg.MainTask, 'lr_asym_ratio', 0.1)
        wd_ratio = getattr(self.cfg.MainTask, 'wd_asym_ratio', 2.0)
        ...
        # v21의 alignment_head도 Standard 그룹에 추가
    ...
    def fusion_train(self, save_path="weights/best_fusion_v22.pt"): 
        ...
        print(f"\n--- Best Validation Performance (v22) ---")
