...
class FusionTrainer(nn.Module, dataProcessor):
    ...
    def _set_task_grad(self, task: str):
        """ 'motion' 또는 'emotion' 태스크에 따라 헤드의 그래디언트를 설정 """
        
        # 행동(Motion) 헤드 파라미터
        for param in self.motion_head.parameters():
            param.requires_grad = (task == 'motion')

        # 감정(Emotion) 헤드 및 정렬(Alignment) 헤드 파라미터
        emotion_params = itertools.chain(
        ...
        for param in emotion_params:
            param.requires_grad = (task == 'emotion')
    ...
    def run_epoch(self, loader, train: bool, current_task: str):
        self.train(train)
        total_loss, num_batches = 0.0, 0
        device = self.cfg.Project.device

        # [수정] 현재 태스크에 맞춰 헤드 동결/해제
        if train:
            self._set_task_grad(current_task)
        ...
        for batch in loader:
            with torch.set_grad_enabled(train):
                ...
                # [수정] 현재 태스크에 따라 다른 손실을 계산
                if current_task == 'motion':
                    ...
                    loss = emo_loss_weight * (loss_v + loss_a) + (loss_align * self.cfg.MainTask.cross_modal_lambda)
    ...
    def fusion_train(self, save_path="weights/best_fusion_v29.pt"):
        ...
        best_score = 0.0 # 교차 학습에서는 보통 전체적인 성능 지표(예: 정확도 평균)를 모니터링
        patience_counter = 0
        best_performance = {}
        ...
        if warmup_epochs > 0:
            print("\n" + "="*50)
            print(f"🚀 STARTING STAGE 1: WARM-UP FOR {warmup_epochs} EPOCHS (ALTERNATING)")
            print("="*50)
            self._set_encoder_grad(requires_grad=False)
            self._create_optimizer_stage1()

            for epoch in range(1, warmup_epochs + 1):
                # [수정] 에폭 홀/짝에 따라 학습 태스크 결정
                current_task = 'motion' if epoch % 2 != 0 else 'emotion'
                print(f"\n[WARM-UP] Epoch {epoch:02d}/{warmup_epochs} | Current Task: {current_task.upper()}")
                
                # [수정] 현재 태스크를 run_epoch에 전달
                tr_loss = self.run_epoch(tr_loader, train=True, current_task=current_task)
                
                # 평가는 모든 태스크에 대해 진행
                va_mot, va_v, va_a = self.evaluate(va_loader)
                # 평가 손실은 현재 태스크 기준으로 계산 (리포팅용)
                va_loss = self.run_epoch(va_loader, train=False, current_task=current_task)

                print(f"  -> Tr Loss({current_task}): {tr_loss:.4f} | Val Loss({current_task}): {va_loss:.4f}")
                print(f"  -> Val Acc(M/V/A): {va_mot:.3f}/{va_v:.3f}/{va_a:.3f}")
                
                current_score = (va_mot + va_v + va_a) / 3

                if current_score > best_score:
                    best_score = current_score
                    patience_counter = 0
                    best_performance = {'mot_acc': va_mot, 'val_acc': va_v, 'aro_acc': va_a}
        ...
        print("\n" + "="*50)
        print(f"🚀 STARTING STAGE 2: FINE-TUNING FOR {total_epochs - warmup_epochs} EPOCHS (ALTERNATING)")
        print("="*50)
        ...
        for epoch in range(warmup_epochs + 1, total_epochs + 1):
            # [수정] 에폭 홀/짝에 따라 학습 태스크 결정
            current_task = 'motion' if epoch % 2 != 0 else 'emotion'
            print(f"\n[FINE-TUNE] Epoch {epoch:02d}/{total_epochs} | Current Task: {current_task.upper()}")
            ...
            if current_score > best_score:
                best_score = current_score
                patience_counter = 0
                best_performance = {'mot_acc': va_mot, 'val_acc': va_v, 'aro_acc': va_a, 'best_val_loss': va_loss}
                torch.save({
        ...
        print(f"\n--- Best Validation Performance (Staged & Alternating Training) ---")
        print(f"Best Avg Accuracy: {best_score:.4f}")
        print(f"Acc(M/V/A): {best_performance.get('mot_acc', 0):.3f}/{best_performance.get('val_acc', 0):.3f}/{best_performance.get('aro_acc', 0):.3f}")
