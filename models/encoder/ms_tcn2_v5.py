...
class AdaptiveDilatedConv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, init_dilation=1, dropout=0.0):
        super().__init__()
...
class MS_TCN2_PG(nn.Module):
    """Adaptive dilation + Temporal Attention ONLY"""
    def __init__(self, num_layers_PG, num_layers_R, num_R, num_f_maps, dim, num_classes,
                 ta_hidden=64, dropout=0.0):
        super().__init__()
        self.PG = Prediction_Generation_TA(num_layers_PG, num_f_maps, dim, num_classes,
                                           ta_hidden, dropout)
        self.Rs = nn.ModuleList([
            copy.deepcopy(Refinement_TA(num_layers_R, num_f_maps, num_classes, num_classes,
                                        ta_hidden, dropout))
...
class Prediction_Generation_TA(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes, ta_hidden, dropout):
        super().__init__()
        self.conv_in = nn.Conv1d(dim, num_f_maps, 1)
        self.ad1 = nn.ModuleList([
            AdaptiveDilatedConv(num_f_maps, num_f_maps, kernel_size=3,
                                 init_dilation=2**(num_layers-1-i), dropout=dropout)
            for i in range(num_layers)
        ])
        self.ad2 = nn.ModuleList([
            AdaptiveDilatedConv(num_f_maps, num_f_maps, kernel_size=3,
                                 init_dilation=2**i, dropout=dropout)
            for i in range(num_layers)
        ])
